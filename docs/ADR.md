...
## ADR-009: Зафіксована версія React 18.2.0 ✅

**Контекст.** У `package.json` версія React була примусово зафіксована на `18.2.0` (хоча раніше використовувалася `^18.3.0`).

**Рішення.** Залишити версію React зафіксованою на рівні `18.2.0` у всіх залежностях монорепозиторію.

**Наслідки.**
- Запобігає потенційним конфліктам типів між `@types/react` (які вирішуються до `18.2.15` у devDependencies) та версією бібліотеки.
- Запобігає попередженням про гідратацію (hydration mismatch) та іншим проблемам сумісності з Material-UI (MUI) 5 та Next.js 14 App Router, які виникають при використанні новіших мінорних версій React 18.3+.
- Забезпечує однакову поведінку застосунку на всіх локальних машинах розробників та CI/CD середовищах.
